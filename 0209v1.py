...
# 組合出影片路徑
VIDEO_FILE = os.path.join(base_path, "media", "0514v1.mp4")
TARGET_REPS_PER_SIDE = 5

# --- 語音系統 (支援即時中斷與統一語速) ---
speech_lock = threading.Lock()
current_engine = None  # 追蹤引擎實例以利中斷
    ...
    def _say():
        ...
        with speech_lock:
            try:
                ...
                engine.setProperty('rate', 200) # 統一語速
...
def run_trainer():
    ...
    if not should_exit:
        ...
        stage = "wait"
        demo_idx = 0
        intro_interrupted = False # 標記開場白是否已因動作中斷

        speak("現在開始復健，我們先從右手手肘開始。舉起手即可開始。")
            ...
            if results.pose_landmarks:
                ...
                if lm[e].visibility > 0.7:
                    ...
                    if user_angle < 45 and stage == "stretch":
                        ...
                        speak(str(counters[current_target])) # 報數功能不受影響
