import base64import htmlimport timeimport threadingfrom dataclasses import dataclassfrom pathlib import Path

import avimport cv2import mediapipe.python.solutions.drawing_utils as mp_drawingimport mediapipe.python.solutions.hands as mp_handsimport mediapipe.python.solutions.pose as mp_poseimport numpy as npimport streamlit as stimport streamlit.components.v1 as componentsfrom streamlit_webrtc import VideoProcessorBase, WebRtcMode, webrtc_streamer

POSE_CONNECTIONS = mp_pose.POSE_CONNECTIONSHAND_CONNECTIONS = mp_hands.HAND_CONNECTIONS

@dataclass(frozen=True)class ExerciseConfig:action_name: strkind: strdemo_video: strtarget_reps: int = 5two_sides: bool = False

def calculate_angle(a, b, c):a, b, c = np.array(a), np.array(b), np.array(c)radians = np.arctan2(c[1] - b[1], c[0] - b[0]) - np.arctan2(a[1] - b[1], a[0] - b[0])angle = abs(radians * 180.0 / np.pi)return 360 - angle if angle > 180.0 else angle

def hand_extension(hand_landmarks):wrist = hand_landmarks.landmark[mp_hands.HandLandmark.WRIST]tips = [mp_hands.HandLandmark.INDEX_FINGER_TIP,mp_hands.HandLandmark.MIDDLE_FINGER_TIP,mp_hands.HandLandmark.RING_FINGER_TIP,mp_hands.HandLandmark.PINKY_TIP,]distances = [np.sqrt((hand_landmarks.landmark[tip].x - wrist.x) ** 2+ (hand_landmarks.landmark[tip].y - wrist.y) ** 2)for tip in tips]return float(sum(distances) / len(distances))

class RehabProcessor(VideoProcessorBase):def init(self, config: ExerciseConfig):self.config = configself.pose = mp_pose.Pose(min_detection_confidence=0.7,min_tracking_confidence=0.6,)self.hands = mp_hands.Hands(min_detection_confidence=0.6,min_tracking_confidence=0.6,)self.lock = threading.Lock()self.stage = "wait"self.reps = 0self.current_target = "RIGHT"self.counters = {"LEFT": 0, "RIGHT": 0}self.status = "按 START 後開始辨識。"self.last_value = "-"self.speech_text = "準備開始復健訓練。"self.speech_id = 1self.last_error_spoken_at = 0.0self.has_started_moving = False
...
def read_processor_state(ctx):if not ctx.video_processor:return {"status": "按 START 後開始辨識。","last_value": "-","reps": 0,"counters": {"LEFT": 0, "RIGHT": 0},"current_target": "RIGHT","speech_text": None,"speech_id": 0,}
...
def render_state(state, config, metrics_slot, status_slot):with metrics_slot.container():if config.two_sides:col1, col2, col3 = st.columns(3)col1.metric("右側", f"{state['counters']['RIGHT']} / {config.target_reps}")col2.metric("左側", f"{state['counters']['LEFT']} / {config.target_reps}")col3.metric("目前", "左側" if state["current_target"] == "LEFT" else "右側")else:col1, col2 = st.columns(2)col1.metric("計次", f"{state['reps']} / {config.target_reps}")col2.metric("狀態", state["last_value"])

status_slot.info(state["status"])

def render_muted_video(video_path):encoded = base64.b64encode(video_path.read_bytes()).decode("ascii")components.html(f"""const video = document.getElementById("demo-video");if (video) {{video.muted = true;video.defaultMuted = true;video.volume = 0;video.onvolumechange = () => {{video.muted = true;video.volume = 0;}};}}""",height=420,)

def install_speech_reader():components.html("""const spoken = new Set();function readSpeechText() {try {const doc = window.parent.document;const node = doc.getElementById("rehab-speech-text");const text = node ? node.textContent.trim() : "";const speechId = node ? node.getAttribute("data-speech-id") : "";if (!text || !speechId || spoken.has(speechId)) return;spoken.add(speechId);const synth = window.parent.speechSynthesis || window.speechSynthesis;if (!synth) return;synth.cancel();const utterance = new SpeechSynthesisUtterance(text);utterance.lang = "zh-TW";utterance.rate = 1.35;synth.speak(utterance);} catch (e) {}}setInterval(readSpeechText, 700);""",height=0,)

def run_app(config: ExerciseConfig):st.set_page_config(page_title=config.action_name, layout="centered")st.title(config.action_name)
